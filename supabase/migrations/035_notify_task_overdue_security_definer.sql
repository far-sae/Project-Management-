-- Fix REST 403 on POST /rpc/notify_task_overdue when:
--   - EXECUTE was never granted on a forked DB, or
--   - notifications INSERT under RLS fails for cross-user rows (invoker) on tightened policies.
-- Aligns with create_notification_v1 (028): SECURITY DEFINER + explicit task visibility (same rules as tasks_locked_visibility).
...
comment on function public.notify_task_overdue(uuid) is
  'Creates at most one task_overdue notification per assignee per UTC day. SECURITY DEFINER with caller task visibility; re-grants EXECUTE for PostgREST.';
